...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            # Load the expert data for initial training data
            with open(load_initial_expertdata, "rb") as fp:
                paths = pickle.load(fp)

            # Note: expert_data doesn't have 'image_obs'
            # random_indices = np.random.permutation(np.arange(len(paths)))[:MAX_NVIDEO]
            # train_video_paths = [paths[i] for i in random_indices]
            # import pdb;pdb.set_trace()
            # for i, pth in enumerate(train_video_paths):
            #     train_video_paths[i] = {k: v[:MAX_VIDEO_LEN] for k, v in pth.items()}
